...
def load_fundamental_data(start_date=default_start_date):
    ...
    macro_data = _get_gdp_and_process(macro_data=macro_data, settings=settings)

    # Process retail data (joining two series)
    macro_data = _process_retail_data(macro_data=macro_data, settings=settings)

    # Get claims data (weekly)
    macro_data, settings = _add_claims_data(macro_data=macro_data,
                                            settings=settings)

    # Clean data
    macro_data = _clean_data(macro_data=macro_data, settings=settings)
...
def _get_gdp_and_process(macro_data=None, settings=None):
...
def _process_retail_data(macro_data=None, settings=None):
...
def _add_claims_data(macro_data=None, settings=None):
...
def _clean_data(macro_data=None, settings=None):
...
def process_factor_model_data(raw_macro_data=None,
                              macro_data=None,
                              settings=None):
    ...
    if 'DTBTM' in settings.data_fields:
        macro_data_1d['DTBTM'][macro_data_1d.index == dt.datetime(2010, 12, 1)] \
            = -0.05
...
def compute_ar_coefs(macro_data=None,
                     macro_data_1d=None,
                     macro_data_3d=None,
                     macro_data_6d=None,
                     macro_data_ma=None,
...
def process_data_by_ar(macro_data=None,
                       ar1_coefs=None,
                       ar1_threshold=0.875,
                       diff_window=1):
...
def compute_factors(macro_data_z=None, settings=None, num_factors=4):
...
def process_factors(components=None, pca_factors=None, settings=None):
...
def retrieve_recession_data(macro_data=None, start_date=None):
...
def estimate_recession_probability(macro_data=None,
                                   macro_indicator=None,
                                   start_date=None):
    import scipy
    from statsmodels.discrete.discrete_model import Logit, Probit
    recession_data, recession_start_dates, recession_end_dates = \
        retrieve_recession_data(macro_data=macro_data,
...
def prepare_volatility_data(ticker='^GSPC',
                            start_date=None,
                            exclude87=False,
...
def prepare_nonparametric_analysis(capped_volatility_lead=None,
